...
最小値クエリを行うx座標の幅をXとして、線分追加をO(log^2 X)・直線追加と最小値計算をO(logX)で行います。     
...
- `add_line(a, b)`は、直線 y = ax + b を追加します。
- `add_line(a, b, xL, xR)`は、**閉区間[xL, xR]に** 線分 y = ax + b を追加します(座標xLとxRを含みます)。  
...
`fold(x: Tx) -> Ty`
- 座標xにおける、現在までに追加した線分(直線)のy座標の最小値を求めます。      
- 座標xにかかる線分(直線)が存在しない場合、かわりにINF (型Tyで表現できる最大の値)を返します。    
ここで、INFの具体的な値は`self.INF`に格納されています。
- 制約: **min_x ≤ x ≤ max_x**
- 計算量: O(logX)
