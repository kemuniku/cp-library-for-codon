...
class SuffixArray:
    '''
    suffix array for codon, PyPy3
    suffix array, LCP arrayのライブラリです。
    '''
    def _suffix_array(A: list[int]) -> list[int]:
        ...
        SuffixArray._SAIS(A, B, bucket, R, mex)
        B.pop()
        return B
    def _SAIS(A: list[int], B: list[int], L: list[int], R: list[int], mex: int) -> None:
        ...
        SuffixArray._induced_sort(A, B, L, R, F, T, LMS, mex)
    ...
    def suffix_array(S: str) -> list[int]:
        ...
        return SuffixArray._suffix_array([ord(Si) for Si in S])
    ...
    def suffix_array(A: list[int]) -> list[int]:
        ...
        if len(A) == 0:
            ...
            return SuffixArray._suffix_array(A[:])
        else:  #Python・PyPy用分岐
            return SuffixArray._suffix_array([ord(Ai) for Ai in A])
    #LCP array
